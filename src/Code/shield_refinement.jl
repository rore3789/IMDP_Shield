...
function find_specific_regions(extents, identifier_values, num_dims, num_regions; selected_dims=nothing)

    refine_regions = []
    dims_refined = Dict()
    for kk in 1:num_regions
        region = extents[kk, :, :]
        for jj in 1:num_dims
            if !isnothing(identifier_values[jj])
                region_bounds = region[jj, :]
                region_low = region_bounds[1]
                region_up = region_bounds[2]

                if region_low <= identifier_values[jj] && region_up >= identifier_values[jj]
                    push!(refine_regions, kk)
                    if !isnothing(selected_dims)
                        dims_refined[kk] = selected_dims  # this is an array! e.g. [1, 3]
                    end
                end
            end
        end
    ...
    return refine_regions, dims_refined
end
...
function refinement_algorithm(refine_states, extents, modes, num_dims, global_dir_name, nn_bounds_dir, refinement;
    ...
    if !individual_nn
        ...
        @info "refinement isn't ready for individual NNs per dimension, exiting"
        exit()
    end
    ...
    elseif predefined_dims
        using_selected_dims = true
        # dims_refined should be defined as an input in this case
    end
    ...
    for mode in modes
        ...
        additive_terms_ = numpy.load(gp_bounds_dir*"/additive_terms_$(mode)_$refinement.npy")
        additive_terms_ = vcat(additive_terms_[keep_states, :, :], additional_array)
        numpy.save(gp_bounds_dir*"/additive_terms_$(mode)_$(refinement+1)", additive_terms_)
